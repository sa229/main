...
Note how components of SSENISUB have well-defined APIs. For example, the API of the `Logic` component is given in the link:{repoURL}/src/main/java/seedu/address/logic/Logic.java[`Logic.java`]
...
Note how SSENISUB app uses Java ``assert``s to verify assumptions.
...
* Add more assertions to Ssenisub as you see fit.
...
Note <<DeveloperGuide#logging, how SSENISUB app uses Java's `java.util.log` package to do logging>>.
...
Add more logging to Ssenisub as you see fit.
...
Note how Ssenisub uses the `ReadOnly*` interfaces to prevent objects being modified by clients who are not supposed to modify them.
...
Analyze SSENISUB code/design to identify,
...
Note <<UsingGradle#, how SSENISUB app uses Gradle to automate build tasks>>.
...
Note <<UsingTravis#, how SSENISUB app uses Travis to perform Continuous Integration>>. (https://travis-ci.org/se-edu/addressbook-level4[image:https://travis-ci.org/se-edu/addressbook-level4.svg?branch=master[Build Status]])
...
* Test methods `prefsReadSave()` and `SsenisubReadSave()` are integration tests. Note how they simply test if The `StorageManager` class is correctly wired to its dependencies.
* Test method `handleSsenisubChangedEvent_exceptionThrown_eventRaised()` is a unit test because it uses _dependency injection_ to isolate the SUT `StorageManager#handleSsenisubChangedEvent(...)` from its dependencies.
