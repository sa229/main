...
through the add command or edit an employee's feedback through the edit command. Every employee added will start off
...
** Each iteration ends when the end of the string is reached or the length of the current substring checked reaches the longest profanity
string length within the profanity list (For increased performance).

* Any leetspeak in the input will be replaced by its appropriate characters before the check, i.e. "H3ll0" will be
corrected to "Hello".
...
To ensure **fast retrieval**, a HashMap is used to store profanities to be rejected from the data file before using it to check if the
...
** Cons: Runs for **O(n^2^)** time, where n is the number of characters in the input.
...
2.  SSENISUB shows a list of staff
3.  User requests to delete a specific staff in the list
4.  SSENISUB deletes the staff
+
Use case ends.
...
// tag::FavouriteStaffTC[]
=== Favouriting a staff

. Favourite a staff while all staff are listed

.. Prerequisites: List all staff using the `list` or `sort` command. Multiple staff in the list.
.. Test case: `fav 3` +
...
.. Prerequisites: List all staff using the `list` or `sort` command. Multiple staff in the list. First staff in the list favourited.
.. Test case: `fav 1` +
   Expected: Staff is already favourited. Error details shown in the status message. Status bar remains the same.
//end::FavouriteStaffTC[]

// tag::UnfavouriteStaffTC[]
=== Unfavouriting a staff

. Unfavourite a staff while all staff are listed

.. Prerequisites: List all staff using the `list` or `sort` command. Multiple staff in the list. One or more staff is favourited.
.. Test case: `unfav 1` +
...
.. Prerequisites: List all staff using the `list` or `sort` command. Multiple staff in the list. No staff in the list already favourited.
.. Test case: `unfav 3` +
   Expected: Staff is already unfavourited. Error details shown in the status message. Status bar remains the same.
//end::UnfavouriteStaffTC[]
