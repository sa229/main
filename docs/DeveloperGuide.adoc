...
2. `AddressBookParser` will receive add as the command and instantiate `AddCommandParser` to parse the rest of the
command.
...
* ** Alternative 2:** `add` is parsed straight from `AddressBookParser`
...
The `unlock` command allows changes made to be saved to the address book.
...
image::UnlockActivityDiagram.png[width="800"]
Figure 3.8.1.1 Activity Diagram for `unlock` command
...
2. Manager searches for a staff
